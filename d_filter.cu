...
void generateGaussianKernel(float* kernel, int radius, float sigma)
{
    ...
    for (int y = -radius; y <= radius; ++y)
    {
        for (int x = -radius; x <= radius; ++x)
        {
            ...
            //printf("kernel %f", kernel[(x + radius) + (y + radius) * kernel_width]);
        }
    }
}
                          ...
                          int num_rows, int num_cols)
{
    ...
    filter_gaussian_1F_kernel<<<grid_sz, block_sz>>>(d_img_out, d_img, d_kernel, radius, sigma_spatial, num_rows, num_cols);
    cudaDeviceSynchronize();
                        ...
                        int num_rows, int num_cols)
{
    ...
    startCudaTimer(&timer);
    filter_gaussian_1F_kernel<<<grid_sz, block_sz>>>(d_img_out, d_img_in, d_kernel, radius, sigma_spatial, num_rows, num_cols);
